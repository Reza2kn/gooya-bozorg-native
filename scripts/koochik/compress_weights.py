...
def main():
 p=argparse.ArgumentParser();p.add_argument('--source',type=Path,required=True);p.add_argument('--out',type=Path,required=True);p.add_argument('--group-size',type=int,default=0);a=p.parse_args();a.out.mkdir(parents=True,exist_ok=True)
 ...
 with (a.out/'step.weights').open('wb') as dest:
  for t in m.graph.initializer:
   ...
   if t.data_type==TensorProto.FLOAT and len(t.dims)==2 and arr.size>=4096:
    view=arr.reshape(arr.shape[0],-1,a.group_size) if a.group_size else arr
    scale=np.maximum(np.max(np.abs(view),axis=-1,keepdims=True)/127,np.finfo(np.float32).tiny).astype(np.float32);q=np.clip(np.rint(view/scale),-127,127).astype(np.int8)
    ...
    receipts.append({'name':t.name,'source_bytes':arr.nbytes,'stored_bytes':q.nbytes+scale.nbytes,'bits':8})
 ...
 (a.out/'compression.json').write_text(json.dumps({'method':'symmetric grouped W8A32, portable dequantization','group_size':a.group_size,'runtime_memory_reduction_claimed':False,'stored_weights_bytes':offset,'quantized_tensors':receipts},indent=2));print('Stored weights bytes',offset)
