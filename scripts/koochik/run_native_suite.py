...
for x in json.loads(a.suite.read_text())['texts']:
 ...
 r=json.loads((out/'report.json').read_text());r['wall_seconds']=time.monotonic()-start;r['bundle_manifest']=json.loads((a.bundle/'manifest.json').read_text());(out/'report.json').write_text(json.dumps(r,indent=2));print(x['id'],r['full_32_step_code_agreement'],r['wall_seconds'],flush=True)
